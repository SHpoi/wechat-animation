...
function abstractTable(){
  ...
  this.trunPageCallBack=function(){}
}
/*
 表格对象模板
 */
...
function table(options){
    ...
    if(this.content != null){
      ...
      for(var i=0;i<len;i++){
        ...
        for(var key in tempDaTa[i]){
          $td = $("<td class='ykytd-"+key+"'>").html(tempDaTa[i][key]).appendTo($tr);
        }
      }
  ...
  this.pagination = function(tdCell){
    ...
    //当前页
    var oA = $("<a/>");
    oA.html((this.currentPageNum*this.displayNum-(this.displayNum-1))+"-"+((this.currentPageNum*this.displayNum)<=(this.allDataNum)?(this.currentPageNum*this.displayNum):(this.allDataNum))+"条，共"+this.allDataNum+"条");
    $td.append(oA);
    //总页
/*    var oA = $("<a/>");
//  oA.attr("href","#1");
//  oA.attr("id","yindex");
    oA.html("共"+(Math.ceil( this.allDataNum/this.displayNum))+"页"+(this.allDataNum)+"条");
  //console.log(this.displayNum);
    ...
    oA.attr("href","#1");
    oA.attr("id","yindex");
    oA.addClass("click-color");
    oA.html("<i class='iconfont icon-shangyiye'></i>首页");
    // $td.append(oA);//实际上要加的
    //上一页
    if(this.currentPageNum>=2){
      ...
      oA.addClass("click-color");
      oA.html("<i class='iconfont icon-gt-copy'></i>上一页");
      $td.append(oA);
    }
    //普通显示格式
    if(this.maxPageNum <= this.groupDataNum){ // 10页以内 为一组
      for(var i = 1;i <= this.maxPageNum ;i++){
        var oA = $("<a/>");
        oA.attr("href","#"+i);
//      oA.addClass("y-option");
          oA.attr("class","y-option");
...
//          oA.addClass("current").siblings().removeClass("current"); 
    ...
    if( (this.maxPageNum - this.currentPageNum) >= 1 ){
      ...
      oA.attr("href","#" + (this.currentPageNum + 1));
      oA.addClass("click-color");
      oA.html("下一页<i class='iconfont icon-iconfonti'></i>");
      $td.append(oA);
    }
    //尾页
    var oA = $("<a/>");
    ...
    oA.addClass("click-color");
    oA.html("尾页<i class='iconfont icon-xiayiye'></i>");
    $td.append(oA);
    ...
    page_a.unbind("click").bind("click",function(){
      ...
      //回调函数
      tempThis.trunPageCallBack.call(this);
      return false;
    });
  }
  //初始化
  this.init = function(begin,end){
    if(options.trunPageCallBack){
      this.trunPageCallBack=options.trunPageCallBack;
    } 
